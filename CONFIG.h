...
#define STEPPER_RUN_DIODE_PIN     A9      // номер пина, куда подключается светодиод индикации работы шагового
...
#define HOLD_ON_STOP  // закомментировать, если не нужно держать обмотки двигателя под напряжение при остановке (режим блокировки)
